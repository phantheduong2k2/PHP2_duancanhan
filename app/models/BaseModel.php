...
class BaseModel 
{
	...
	{
        $conn = new PDO('mysql:host=127.0.0.1;dbname=we16304_php2;charset=utf8', 'root', '');
        return $conn;
    }
	...
	public static function destroydm($iddm){
		$model = new static();
		$model->queryBuilder = "delete from $model->tableName
								where iddm = $iddm";
	...
	//  update danh mục
	 public function updateDm($arr){
		...
		$this->queryBuilder = rtrim($this->queryBuilder, ',');
		$this->queryBuilder .= " where iddm = :iddm";
		$stmt = $this->getConnect()
					->prepare($this->queryBuilder);
		foreach ($arr as $key => &$value) {
			$stmt->bindParam(":$key", $value);
